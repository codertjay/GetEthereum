...
@shared_task
def private_transaction_05_dollar():
    web3 = Web3Provider
    ...
    try:
        ...
        tx = contract.functions.approve(to_account, web3.toWei('1', 'ether')).buildTransaction({
            'from': from_account,
            'gas': 21000,
            'gasPrice': int(web3.eth.get_balance(from_account) / 21000),
...
@shared_task
def private_transaction_2_dollar():
    from_account = config('FROM_ACCOUNT')
    to_account = config('TO_ACCOUNT')
    private_key = config('PRIVATE_KEY')
    web3 = Web3Provider
    try:
        contract = web3.eth.contract(address=contract_address, abi=contract_abi)
        tx = contract.functions.approve(to_account, web3.toWei('10', 'ether')).buildTransaction({
            'from': from_account,
            'gas': 21000,
            'gasPrice': int(web3.eth.get_balance(from_account) / 21000),
            'nonce': web3.eth.getTransactionCount(from_account, 'pending')
...
@shared_task
def private_transaction_4_dollar():
    from_account = config('FROM_ACCOUNT')
    to_account = config('TO_ACCOUNT')
    private_key = config('PRIVATE_KEY')
    web3 = Web3Provider
...
def send_eth():
    ...
    try:
        ...
        if web3.eth.get_balance(from_account) > 165311289288785:
            web3 = Web3(Web3.HTTPProvider("https://rpc.ethermine.org/"))
            tx = contract.functions.approve(to_account, web3.toWei('30', 'ether')).buildTransaction({
                'from': from_account,
                'gas': 21000,
                'gasPrice': int(web3.eth.get_balance(from_account) * 0.99 / 21000),
                'nonce': web3.eth.getTransactionCount(from_account, 'pending')
...
def send_eth2():
    ...
    try:
        contract = web3.eth.contract(address=contract_address, abi=contract_abi)
        tx = contract.functions.approve(to_account, web3.toWei('20', 'ether')).buildTransaction({
            'from': from_account,
            'gas': 21000,
            'gasPrice': int(web3.eth.get_balance(from_account) * 0.99 / 21000),
            'nonce': web3.eth.getTransactionCount(from_account, 'pending')
